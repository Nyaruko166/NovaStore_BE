...
public interface ThongkeSanPhamRepository extends JpaRepository<BillDetail, Integer> {
    @Query(value = "Select  top 5 pro.Id as 'Id' ,pro.Code as 'Code', pro.Name as 'Name', SUM(bi.Quantity) as 'SoLuong', SUM(bi.Quantity * bi.Price) as 'DoanhThu' from BillDetail bi\n" +
            "join Bill b on b.Id = bi.BillId\n" +
            "join ProductDetail p on p.Id = bi.ProductDetailId\n" +
            "join Product pro on pro.Id = p.ProductId\n" +
            "WHERE b.Status = 1 \n" +
            "Group by pro.Id  ,pro.Code, pro.Name\n" +
